...
public interface MovieRepository
    ...
    @Query(value = "SELECT * FROM movies movie "
            ...
            + "AND (movie.description LIKE %:description%)",
            nativeQuery = true)
    List<Movie>
    ...
    Optional<Movie> findByNameAndYear(
            String name, Integer year);

    List<Movie> findByNameInAndYearIn(
            List<String> names, List<Integer> years);

    Page<Movie> findAll(Pageable pageable);

    @Modifying
    @Query(nativeQuery = true,
            value = "DELETE FROM movies WHERE id = ?")
